function [pinwheel_stats,pinwheel_spurious] = get_Wallaby_pinwheel_stats(AllMaps,tracker_obj,simple_track)
%{
...
boot_samples = size(AllMaps,3);
ROI= ones(size(AllMaps,[1,2]));
...
base = 1;
z_base = AllMaps(:,:,base);
pinwheel_tables{base} = tracker_obj.find_pinwheels(z_base,ROI,base);
...
for input=2:boot_samples
    z_input = AllMaps(:,:,input);
    
    % get pinwheels in input sample and fill tracking tables
    pinwheel_tables{input}=tracker_obj.find_pinwheels(z_input,ROI,base);
    ...
    tracking = tracker_obj.interpolate(z_base,z_input,ROI);
