function h = oiPlotDRMap(images, varargin),
...
%images = abs(images);
for idx = 1:numConds,
  x(:,:,idx) = images(:,:,idx) .* cos((idx-1)*4*(pi/numConds));
  y(:,:,idx) = images(:,:,idx) .* sin((idx-1)*4*(pi/numConds));
end
