%function read_Wallaby_data()
    ...
    % data frames to use
    %frames_to_use = {6:10; ... % 1 sec before stim onset (blank)
     frames_to_use = {   26:30; 31:35;... % 2 sec before stim offset %%%after stim onset
...
%     blanks = data(:,:,:,:,1);
%     data(:,:,:,:,1) = [];
blank_frames_to_use = 10:50;
blank = zeros([pix_x, pix_y,num_blocks]);
...
%                 assert(numel(frame_list)>4, 'Not enough images to make an averaged frame.')
        % read frames
        trial=ReturnTrial(dimg,block_ii,1);
        % add average
        blank(:,:,stim_ii,block_ii) = mean(trial(:,:,blank_frames_to_use),3);
