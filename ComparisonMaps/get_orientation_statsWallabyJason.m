function orientation_stats = get_orientation_statsWallabyJason(AllMaps,JackknifeSample,alpha)
%{
...
% boot_samples
num_boot_samples = size(AllMaps,3);
%disp(num_boot_samples)
...
bootstat = zeros(sum(ROI,'all'),num_boot_samples);
...
jackstat = zeros(sum(ROI,'all'),size(JackknifeSample,3));
for ii=1:size(JackknifeSample,3)
    jackstat(:,ii) = JackknifeSample(ROI,ii);
end
...
orientation_stats = cat(3,...
    vector2array(sel_ci(:,1).*exp(1i*ori_ci(:,1)),ROI),...
    vector2array(stat,ROI),...
    vector2array(sel_ci(:,2).*exp(1i*ori_ci(:,2)),ROI));

end


function map = vector2array(map,ROI)
    if ~isvector(map)
        disp('WARNING: Input map is not a vector.')
        return
    else
        %disp(sum(ROI(:)))
        if length(map)~=sum(ROI(:),'all')
