function checkPinwheelDensityJason(experiment_num_list,animal,AnimalDataFolder,DataFolderMain,...
    ...
    close all
    addpath '~/Cloud/Cloud/PhD/data/data share/Wallaby data/Script'
...
%     highpass_mm = 0.56;
    ...
    FigureFile1 = [FigureFolder animal '_PinwheelsJasonGIF' num2str(GIF_SNTH)];    
    FigureFile2 = [FigureFolder animal '_ColumnSpacingJasonGIF' num2str(GIF_SNTH)];
...
%         z = z - imfilter(z,h,'symmetric','conv'); 
% 
%         % lowpass
%         sigma = 8;
%         h = fspecial('gaussian', 6*sigma, sigma);
%         z = imfilter(z,h,'symmetric','conv'); 
...
%         z = re + 1i*im;
        ...
        load([data_path 'fmaps_' data_info.ID(end) '.mat'])
        [op,r] = oiCalcORMap(real(fmaps), [0:7]*pi/8, 'vector');
        z = r.*exp(2i.*op);
        z = double(z);
        ROI = ones(size(z));
        ...
        [pinw, windno] = locate_pinwheels(z);

        %% get column spacing Jason
        op_wl = mean([fourier_wavelength(real(z)), fourier_wavelength(imag(z))]);

        %% calc pinwheel density Jason
        npinw = size(pinw,1);

        % Calculate per pixel
        density_per_pixel = npinw/sum(ROI(:));
        ...
        [average_spacing_mmManuel,local_spacing_mmManuel,~,WavletCoefficient] = get_column_spacingManuel(z,ROI,data_info.pix_per_mm,smallest_w_mm,largest_w_mm,w_step_mm);
        [average_spacing_mm,local_spacing_mm] = get_column_spacing(z,ROI,data_info.pix_per_mm,smallest_w_mm,largest_w_mm,w_step_mm,0);
        PwInfo.average_spacing_mm = average_spacing_mmManuel;
        PwInfo.NumHypercolumns = sum(ROI,'all')/(data_info.pix_per_mm*PwInfo.average_spacing_mm)^2;
        ...
        sigma = 0.1;
        PwInfo.LocalPwDensityFixedFilter=getLocalPwDensityFixedFilter(data_info,ROI,PwInfo,local_spacing_mmManuel,sigma);
...
%         %% calculate power profile 
        ...
        power_profile_filtered = define_filter_settings(data_info,ROI,z,power_spectrum_range_mm);
%         power_profile = define_filter_settings(data_info,ROI,data_obj.read_map(),power_spectrum_range_mm);
...
function LocalPwDensityFixedFilter = getLocalPwDensityFixedFilter(data_info,ROI,PwInfo,local_spacing_mm,sigma)
    disp('calc LocalPwDensity with FixedFilter')
    average_spacing_mm = mean(local_spacing_mm(ROI),'all');
    local_pw_dens = put_gaussians(size(ROI,1),size(ROI,2), PwInfo.PWxList, PwInfo.PWyList,average_spacing_mm*data_info.pix_per_mm,sigma,ROI);
    local_pw_dens = local_pw_dens./sum(local_pw_dens(ROI)).*PwInfo.NumberPw;
    LocalPwDensityFixedFilter = local_pw_dens.*(local_spacing_mm*data_info.pix_per_mm).^2;
end
