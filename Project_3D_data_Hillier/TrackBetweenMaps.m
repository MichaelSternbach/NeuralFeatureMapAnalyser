...
for ii = 1:N%length(data_info.exp_data)
    if ~isempty(data_info.exp_data(ii).binocular)
        %% load data
        data_data_count=data_data_count+1;
        [data_objs{data_data_count},~,~]=loadMouseLemurData(data_info,data_path,ii,1);
        data_objs{data_data_count}.prepare_samples_array(bootstrapsamples)
        disp(['loaded experiment' num2str(ii)])

        z = data_objs{data_data_count}.filter_map(data_objs{data_data_count}.read_map());
        [~,~,~,pinwheel_pos{data_data_count}.x,pinwheel_pos{data_data_count}.y,~, ~] = find_pinwheels(z,0,data_objs{data_data_count}.ROI,0);
        disp(['finished pinwheel finding experiment' num2str(ii)])

        %% track pinwheels between maps
        if data_count>2
            z1 = data_objs{data_data_count-1}.filter_map(data_objs{data_count-1}.read_map());
        ...
        StatsOPMs{data_data_count} = getStatsOPM(data_objs{data_data_count});
