function plotCITest(CI,DoFilter,data_obj,folder,CovAligned,alpha)
    ...
    plotAbsTile(CI_angle, 'CI angle var [°]',maxMap,minMap,ROI)
    ...
    plotAbsTile(CI_Abs, 'CI abs var',maxMap,minMap,ROI)

    if DoFilter
            CIFile = [folder 'Filtered_CI_test' data_obj.info.ID];
        else
            CIFile = [folder 'CI_test' data_obj.info.ID];
    end
