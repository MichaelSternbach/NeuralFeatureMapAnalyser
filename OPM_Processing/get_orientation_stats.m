function [orientation_stats,bs_samples,jk_samples] = get_orientation_stats(data_obj,alpha,apply_filter,direction_map)
    ...
    %% if two outputs convert bootstat to array
    if nargout == 2
        bs_samples = zeros(size(data_obj.data,1),size(data_obj.data,2),size(bootstat,2));
        for ii=1:size(bootstat,2)
            bs_samples(:,:,ii) = data_obj.vector2array(bootstat(:,ii));
        end
    end

    %% if three outputs convert jackstat to array
    if nargout == 3
        jk_samples = zeros(size(data_obj.data,1),size(data_obj.data,2),size(data_obj.data,1),size(jackstat,2));
        for ii=1:size(jackstat,2)
            jk_samples(:,:,ii) = data_obj.vector2array(jackstat(:,ii));
        end
    end
